...
def train(model, cfg, model_cfg):
    cfg.batch_size = 64 if cfg.batch_size < 1 else cfg.batch_size
    ...
    train_set = LidcDataset(
        cfg.LIDC_256_PATH, points_sampler=points_sampler,# augmentator=train_augmentator
    )
    val_set = LidcDataset(
        cfg.LIDC_256_VAL_PATH, points_sampler=points_sampler, #augmentator=val_augmentator
    )
