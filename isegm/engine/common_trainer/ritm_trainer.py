...
class RITMTrainer(ISTrainer):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            wandb.log({'clicks_before_backprop': num_iters,
                       'average_point_size_before_backprop': torch.mean(torch.tensor([p for p in points[:,:,-1].flatten() if p > 0]))})
        ...
        for bindx in range(fn_mask.shape[0]):
            ...
            if len(indices) > 0:
                ...
                if is_positive:
                    ...
                    points[bindx, 2 * num_points - click_indx, 3] = float(5) # static disk radius
