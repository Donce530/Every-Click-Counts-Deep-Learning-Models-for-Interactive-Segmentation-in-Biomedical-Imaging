...
class IterativeFocalClickTrainer(ISTrainer):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            focus_images, focus_rois, instances_focus, trimap_focus = (
                batch_data["images_focus"].to(self.device),
                batch_data["rois"].to(self.device),
                batch_data["instances_focus"].to(self.device),
                batch_data["trimap_focus"].to(self.device),
                
            )
            full_features, full_logits = output["feature"], output["instances"]
            ...
            hc, wc = y2 - y1, x2 - x1
            ry, rx = h / hc, w / wc
            bias = torch.stack([y1, x1, torch.zeros_like(y1), torch.zeros_like(y1)], dim=1).unsqueeze(1)
            ratio = torch.stack([ry, rx, torch.ones_like(ry), torch.ones_like(ry)], dim=1).unsqueeze(1)
            bias = bias.to(self.device)
            ratio = ratio.to(self.device)
            focus_points = (backprop_points - bias) * ratio
            
            
            refine_output = self.net.refine(
                focus_images, focus_points, full_features, full_logits, bboxes
            )
            ...
            loss = self.add_loss(
                ...
                losses_logging,
                validation,
                lambda: (refine_output['trimap'], trimap_focus),
            )
            loss = self.add_loss(
