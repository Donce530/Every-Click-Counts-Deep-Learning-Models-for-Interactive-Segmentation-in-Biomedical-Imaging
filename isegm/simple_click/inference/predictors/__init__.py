...
from .base_explainable import BasePredictorExplainable
...
from isegm.simple_click.inference.transforms import ZoomIn
...
def get_predictor(
    ...
    elif brs_mode == 'NoBRSExplainable':
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = BasePredictorExplainable(
            net, device, zoom_in=zoom_in, with_flip=with_flip, **predictor_params_
        )
