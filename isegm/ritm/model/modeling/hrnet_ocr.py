...
class HighResolutionNet(nn.Module):
    def __init__(
        ...
        #Stacked
        # self.conv1 = nn.Conv2d(num_channels + 3, 64, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    def forward(self, x, additional_features=None):
        feats, pre_stage_feats = self.compute_hrnet_feats(x, additional_features)
        if self.ocr_width > 0:
            ...
            return [out, out_aux, pre_stage_feats]
        else:
            return [self.cls_head(feats), None, pre_stage_feats]

    def compute_hrnet_feats(self, x, additional_features):
        pre_stage_features = self.compute_pre_stage_features(x, additional_features)
        x = self.layer1(pre_stage_features)
        ...
        return self.aggregate_hrnet_features(x), pre_stage_features
    ...
    def compute_pre_stage_features(self, x, additional_features):
        ...
        # Concat
        # if additional_features is not None:
        #     x = torch.concat((x, additional_features), axis=1)
        x = self.conv2(x)
        x = self.bn2(x)
        return self.relu(x)
