...
# from isegm.ritm.model.ops import DistMaps, ScaleLayer, BatchImageNormalize
from isegm.ritm.model.ops import ScaleLayer, BatchImageNormalize
...
class ISModel(nn.Module):
            ...
            for click_radius in self.clicks_groups:
                self.dist_maps.append(
                    DistMaps(
                        ...
                        spatial_scale=1.0,
                        dynamic_radius=dist_map_mode,
                        overwrite_maps=overwrite_click_maps
                    )
                )
        else:
    ...
    def forward(self, image, points):
        ...
        if self.rgb_conv is not None:
            ...
            # # Stacked
            # inputs = torch.concat((image, coord_features), dim=1)
            # outputs = self.backbone_forward(inputs)
            
            #SANITY CHECK HACK
            # print(image.shape, prev_mask.shape, prev_mask.sum())
            # coord_features = self.maps_transform(prev_mask)  # prev mask holds gt
            # outputs = self.backbone_forward(image, coord_features) # coord features is transformed gt
