...
from isegm.ritm.utils.misc import get_labels_with_sizes
from isegm.ritm.data.transforms import remove_image_only_transforms
...
class DSample:
    ...
    def get_object_mask(self, obj_id):
        ...
        obj_mask = (
            (self._encoded_masks[:, :, layer_indx] == mask_id).astype(np.int32)
            if self._binary_mask
            else self._encoded_masks[:, :, layer_indx]
        )
