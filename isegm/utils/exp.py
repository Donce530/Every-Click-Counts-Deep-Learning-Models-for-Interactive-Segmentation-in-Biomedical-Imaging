...
def init_experiment(cfg, model_name):
    ...
    experiments_path = Path(cfg.data_paths.EXPS_PATH)
    ...
    exp_path = Path(hydra_cfg['runtime']['output_dir'])

    cfg.data_paths.EXP_PATH = exp_path
    cfg.data_paths.CHECKPOINTS_PATH = exp_path / 'checkpoints'
    cfg.data_paths.VIS_PATH = exp_path / 'vis'
    cfg.data_paths.LOGS_PATH = exp_path / 'logs'

    if cfg.local_rank == 0:
        cfg.data_paths.LOGS_PATH.mkdir(exist_ok=True)
        cfg.data_paths.CHECKPOINTS_PATH.mkdir(exist_ok=True)
        cfg.data_paths.VIS_PATH.mkdir(exist_ok=True)

        dst_script_path = exp_path / (
            model_path.stem
            + datetime.strftime(datetime.today(), '_%Y-%m-%d-%H-%M-%S.py')
        )
        if cfg.temp_model_path:
            shutil.copy(cfg.temp_model_path, dst_script_path)
            os.remove(cfg.temp_model_path)
        else:
    ...
    if cfg.local_rank == 0:
        add_logging(cfg.data_paths.LOGS_PATH, prefix='train_')
