...
def save_checkpoint(net, checkpoints_path, best=False, verbose=True, multi_gpu=False, prefix=''):
    if prefix != '':
        prefix = f'{prefix}_'

    if best:
        checkpoint_name = f'{prefix}best_model.pth'
    else:
        checkpoint_name = f'{prefix}last_checkpoint.pth'
    ...
    if verbose:
        logger.info(f'Save {checkpoint_name} to {str(checkpoint_path)}')
