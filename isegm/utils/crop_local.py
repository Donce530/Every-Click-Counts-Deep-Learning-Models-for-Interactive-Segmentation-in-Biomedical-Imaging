...
def map_point_in_bbox(y, x, disk_radius, y1, y2, x1, x2, crop_l):
    h, w = y2 - y1, x2 - x1
    ...
    y = (y - y1) * ry
    x = (x - x1) * rx
    disk_radius = disk_radius * avg_ratio
    return y, x, disk_radius


# ADDED min crop
def get_focus_cropv1(pred_mask, previous_mask, global_roi, y, x, ratio, min_crop=0):
    ...
    y1, y2, x1, x2 = expand_bbox(pred_mask, y1, y2, x1, x2, ratio, min_crop)
    y1 = max(y1, ymin)
    y2 = min(y2, ymax)
    x1 = max(x1, xmin)
    x2 = min(x2, xmax)
    ...
    #     print("Step 1: Check initial inputs")
    #     print(f"   pred_mask: {pred_mask.sum()}")
    #     print(f"   previous_mask: {previous_mask.sum()}")
    #     print(f'   global_roi: {global_roi}')
    #     print(f'   y: {y}, x: {x}')
    #     print(f'   ratio: {ratio}')

    #     if previous_mask.sum() == 0:
    #         print("Step 2: No previous mask detected")
    #         y1, y2, x1, x2 = get_bbox_from_mask(pred_mask)
    #     else:
    #         print("Step 2: Detecting connected components")
    #         num, labels = cv2.connectedComponents(diff_regions.astype(np.uint8))
    #         label = labels[y, x]
    #         print(f"   Num connected components: {num}")
    #         print(f"   Labels: {labels}")
    #         print(f"   Target label at (y, x): {label}")

    #         diff_conn_mask = labels == label
    #         print(f"   Diff connected mask: {diff_conn_mask}")
    ...
    #         if hd < hp / 3 or wd < wp / 3:
    #             print("Step 3: HD and WD are small, using alternate values")
    #             r = 0.2
    #             l = max(hp, wp)
    #             y1, y2, x1, x2 = y - r * l, y + r * l, x - r * l, x + r * l
    #         else:
    #             print("Step 3: HD and WD are sufficient, using connected component values")
    #             y1, y2, x1, x2 = y1d, y2d, x1d, x2d

    #     print("Step 4: Expanding the bbox")
    #     y1, y2, x1, x2 = expand_bbox(pred_mask, y1, y2, x1, x2, ratio)
    #     print(f"   Expanded Bbox: (y1, y2, x1, x2) = ({y1}, {y2}, {x1}, {x2})")

    #     print("Step 5: Clipping bbox to global_roi")
    #     y1 = max(y1, ymin)
    #     y2 = min(y2, ymax)
    #     x1 = max(x1, xmin)
    #     x2 = min(x2, xmax)
    #     print(f"   Clipped Bbox: (y1, y2, x1, x2) = ({y1}, {y2}, {x1}, {x2})")

    #     print("Step 6: Returning the final bbox coordinates")
    return y1, y2, x1, x2

    # def get_focus_cropv1(pred_mask, previous_mask, global_roi, y, x, ratio):
    pred_mask = pred_mask > 0.49
    # print("pred_mask > 0.49:", pred_mask)

    previous_mask = previous_mask > 0.49
    # print("previous_mask > 0.49:", previous_mask)
    ...
    diff_regions = np.logical_xor(previous_mask, pred_mask)
    # print("diff_regions:", diff_regions)

    # np.set_printoptions(threshold=sys.maxsize)
    # pred_mask_str = np.array2string(pred_mask, separator=',')
    # print("pred_mask:", pred_mask_str)

    if previous_mask.sum() == 0:
        y1, y2, x1, x2 = get_bbox_from_mask(pred_mask)
        print("y1, y2, x1, x2 from pred_mask:", y1, y2, x1, x2)
    else:
        num, labels = cv2.connectedComponents(diff_regions.astype(np.uint8))
        # print("num:", num)
        # print("labels:", labels)
        ...
        diff_conn_mask = labels == label
        # print("diff_conn_mask:", diff_conn_mask)

        y1d, y2d, x1d, x2d = get_bbox_from_mask(diff_conn_mask)
        print("y1d, y2d, x1d, x2d from diff_conn_mask:", y1d, y2d, x1d, x2d)
        ...
        y1p, y2p, x1p, x2p = get_bbox_from_mask(pred_mask)
        print("y1p, y2p, x1p, x2p from pred_mask:", y1p, y2p, x1p, x2p)
        ...
        if hd < hp / 3 or wd < wp / 3:
            ...
            l = max(hp, wp)
            y1, y2, x1, x2 = y - r * l, y + r * l, x - r * l, x + r * l
            print("Adjusted bbox (hd < hp/3 or wd < wp/3):")
        else:
            y1, y2, x1, x2 = y1d, y2d, x1d, x2d
            print("Original bbox (hd >= hp/3 and wd >= wp/3):")

    print("Final bbox before expansion and boundary checks:")
    print("y1:", y1)
    print("y2:", y2)
    print("x1:", x1)
    print("x2:", x2)
    ...
    x1 = max(x1, xmin)
    x2 = min(x2, xmax)
    print("Final bbox after boundary checks:")
    print("y1:", y1)
    print("y2:", y2)
    print("x1:", x1)
    print("x2:", x2)
...
def PointsInBox(points, y1, y2, x1, x2, H, W):
    ...
    for i in range(num_pos):
        ...
        if y > y1 and y < y2 and x > x1 and x < x2:
            ...
            (
                new_points[valid_pos, 0],
                new_points[valid_pos, 1],
                new_points[valid_pos, 2],
            ) = (new_y, new_x, index)
            valid_pos += 1
        if y == -1:
            break
    ...
    for i in range(num_pos, num_pos * 2):
        ...
        if y > y1 and y < y2 and x > x1 and x < x2:
            new_x, new_y = (x - x1) * x_ratio, (y - y1) * y_ratio
            (
                new_points[valid_neg, 0],
                new_points[valid_neg, 1],
                new_points[valid_neg, 2],
            ) = (new_y, new_x, index)
            valid_neg += 1
        if y == -1:
            break
