...
class ISDataset(torch.utils.data.dataset.Dataset):
        ...
        while 1:
            try:
                ...
                if self.with_refiner:
                    ...
                    hc, wc = y2 - y1, x2 - x1
                    ry, rx = h / hc, w / wc
                    bias = np.array([y1, x1, 0, 0])
                    ratio = np.array([ry, rx, 1, 1])
                    points_focus = (points - bias) * ratio

                    if (
                        mask.sum() > self.min_object_area
                        and mask.sum() < mask_area * 0.85
                    ):
                        output = {
                        ...
                        print(f'index: {index}')
                        print(
                            f'mask sum > min object area: {mask.sum()} {self.min_object_area} {mask.sum() > self.min_object_area}'
                        )
                        print(
                            f'mask sum < mask area * 0.85: {mask.sum()} {mask_area * 0.85} {mask.sum() < mask_area * 0.85}'
                        )
    ...
    def sampling_roi_full_object(self, gt_mask, min_size=32):
        ...
        y1, y2, x1, x2 = expand_bbox_with_bias(
            gt_mask, y1, y2, x1, x2, ratio, min_size, 0.3
        )
        return y1, x1, y2, x2
