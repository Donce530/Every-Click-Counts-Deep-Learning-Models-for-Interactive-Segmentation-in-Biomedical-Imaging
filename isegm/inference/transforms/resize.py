...
class ResizeTrans(BaseTransform):
    def __init__(self, l=480, recompute_click_size_on_zoom=None):
        super().__init__()
        self.crop_height = l
        self.crop_width = l
        ...
        if self.recompute_click_size_on_zoom is None:
            raise Exception('recompute_click_size_on_zoom is not set. It has to be true for dynamic clicks and false for static clicks')
    ...
    def transform(self, image_nd, clicks_lists):
        ...
        for clicks_list in clicks_lists:
            ...
            for click in clicks_list:
                resized_coords = (click.coords[0] * y_ratio, click.coords[1] * x_ratio)
                disk_radius = click.disk_radius * avg_ratio if self.recompute_click_size_on_zoom else click.disk_radius
                clicks.append(click.copy(coords=resized_coords, disk_radius=disk_radius))
            clicks_lists_resized.append(clicks)
