...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # Ensure that the inputs are float type and apply sigmoid
        inputs = inputs.float()
...
class DiceLossSigmoid(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # Ignore certain labels if specified
...
class BinaryCrossEntropyLossSigmoid(nn.Module):
    def __init__(self):
        super(BinaryCrossEntropyLossSigmoid, self).__init__()
...
class ClickDiceLoss(nn.Module):
    ...
    def forward(self, pred, target, click_map):
        ...
        intersection = (pred * target).sum(dim=self.batch_axis)
        union = pred.sum(dim=self.batch_axis) + target.sum(dim=self.batch_axis)

        # Adjust union for oversegmentation penalty
        # Only penalize false positives in the 'shouldnt_segment' region
        penalty = (pred * shouldnt_segment).sum(dim=self.batch_axis)
        adjusted_union = union + penalty

        # Calculate weighted Dice loss
        dice_loss = 1 - (2 * intersection + self.eps) / (adjusted_union + self.eps)
