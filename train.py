...
def main(cfg: DictConfig):
    if cfg.temp_model_path:
        model_script = load_module(cfg.temp_model_path)
    else:
        model_script = load_module(cfg.model_path)

    model_base_name = getattr(model_script, 'MODEL_NAME', None)

    cfg.distributed = 'WORLD_SIZE' in os.environ
    cfg = init_experiment(cfg, model_base_name)
...
def parse_args():
    ...
    #     parser.add_argument(
    #         "--dataset", type=str, help="Dataset to use for training.", default='LIDC-2D'
    #     )
